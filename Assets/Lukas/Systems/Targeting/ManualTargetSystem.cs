...
partial struct ManualTargetSystem : ISystem
{
    float lastClickTime;
    float angleOffset;
        ...
        if (Input.GetMouseButtonUp(1))
        {
            ...
            if (collisionWorld.CastRay(raycastInput, out Unity.Physics.RaycastHit raycastHit))
            {
                ...
                foreach (var (unitTargetsNetcode, dotRef, unitEntity) in
                    SystemAPI.Query<RefRW<UnitTargetsNetcode>, RefRW<MovementDotRef>>().WithAll<GhostOwnerIsLocal, Selected>().WithEntityAccess())
                {
                    if (unitEntity == hitEntity) continue; // ignore self-target
                    ...
                    unitTargetsNetcode.ValueRW.requestDestinationEntity = hitEntity;
            ...
            else
            {
                ...
                {
                    unitTargetsNetcode.ValueRW.requestDestinationEntity  = Entity.Null;
